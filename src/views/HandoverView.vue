...
const callingOrders = computed(() =>
  isDemo.value ? demoStore.callingOrders : orderStore.callingOrders
)
...
<template>
  <div class="min-h-screen bg-gray-50 flex flex-col" style="padding-bottom: calc(3.5rem + env(safe-area-inset-bottom))">
    <AppHeader title="📦 受渡し" />

    <!-- 受渡し待ちリスト -->
    <div class="flex-1 overflow-y-auto p-4">
      <div v-if="callingOrders.length === 0" class="text-center text-gray-400 py-16 text-xl">
        受渡し待ちの注文はありません
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          v-for="order in callingOrders"
          :key="order.id"
        ...
        >
          <div class="text-center mb-4">
            ...
            <StatusBadge status="calling" large class="mt-2" />
          </div>
